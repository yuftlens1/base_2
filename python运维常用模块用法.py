...
'''
把内存空间比作一个个箱子，箱子上能贴标签。
浅拷贝就相当于你在同一个箱子上多贴一个标签，不管你用标签1还是标签2 都能找到这个箱子，且对这个箱子里的东西做的改动都是同步的。
而深拷贝就相当于你把整个箱子复制了一份，然后你现在有一个贴着标签1的箱子和一个贴着标签2的箱子，对任意一个箱子做的操作都是独立的，不会影响到另外一个箱子
